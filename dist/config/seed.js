...
async function updateProductMakeModel(sku, make, model) {
...
async function resolveMakeId(name) {
    const row = await database_1.db
        .select({ id: schema_1.makes.id })
        .from(schema_1.makes)
        .where((0, drizzle_orm_1.eq)(schema_1.makes.name, name))
        .get();
    return row?.id ?? null;
}
async function resolveModelId(name, makeId) {
    const row = await database_1.db
        .select({ id: schema_1.models.id })
        .from(schema_1.models)
        .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.models.name, name), (0, drizzle_orm_1.eq)(schema_1.models.make_id, makeId)))
        .get();
    return row?.id ?? null;
}
async function blogExists(postSlug) {
    const row = await database_1.db
        .select({ id: schema_1.blogPosts.id })
        .from(schema_1.blogPosts)
        .where((0, drizzle_orm_1.eq)(schema_1.blogPosts.slug, postSlug))
        .get();
    return row !== undefined;
}
// ─── Engine Data ──────────────────────────────────────────────────────────────
const engines = [
    {
        ...
        quantity: 3,
        featured: true,
        short_description: 'JDM Honda K24A 2.4L inline-4 engine sourced from Japanese domestic market Accord. ' +
            'Known for its VTEC reliability and high redline performance.',
        description: 'The Honda K24A is a 2.4-liter inline-4 engine that represents the pinnacle of Honda\'s ' +
        ...
        meta_description: 'Buy a genuine JDM Honda K24A 2.4L engine from 2008-2012 Accord. Low-mileage, inspected, and ready to ship.',
    },
    {
        sku: 'ENG-2JZGTE-001',
        ...
        quantity: 2,
        featured: true,
        short_description: 'The legendary Toyota 2JZ-GTE twin-turbocharged inline-6. Straight from JDM Supra units, ' +
            'renowned for its ability to handle 1000+ hp builds.',
        description: 'The Toyota 2JZ-GTE is arguably the most legendary JDM engine ever produced. ' +
        ...
        meta_description: 'Legendary Toyota 2JZ-GTE twin-turbo 3.0L JDM engine from 1993-1998 Supra. Ready for high-horsepower builds.',
    },
    {
        sku: 'ENG-SR20DET-001',
        ...
        quantity: 4,
        featured: true,
        short_description: 'Nissan SR20DET 2.0L turbocharged inline-4 from JDM Silvia. The go-to engine for drift builds and track cars.',
        description: 'The Nissan SR20DET is the definitive JDM drift engine. Originally installed in the Nissan Silvia S13, S14, and S15, ' +
        ...
        meta_description: 'Genuine JDM Nissan SR20DET 2.0L turbo engine from Silvia. Perfect for drift builds. Low mileage, inspected.',
    },
    {
        sku: 'ENG-EJ257-001',
        ...
        quantity: 3,
        featured: false,
        short_description: 'Subaru EJ257 2.5L turbocharged boxer engine from JDM WRX STI. Delivers rally-proven performance.',
        description: 'The Subaru EJ257 is the high-performance turbocharged variant of Subaru\'s EJ25 boxer engine, ' +
        ...
        meta_description: 'JDM Subaru EJ257 2.5L turbo boxer engine from WRX STI. Factory forged internals. Low mileage imported units.',
    },
    {
        sku: 'ENG-13BREW-001',
        ...
        quantity: 2,
        featured: true,
        short_description: 'Mazda 13B-REW sequential twin-turbo rotary engine from JDM FD RX-7. Iconic performance, lightweight design.',
        description: 'The Mazda 13B-REW is the crown jewel of Mazda\'s rotary engine lineage. Used in the FD3S RX-7 from 1993–2002, ' +
        ...
        meta_description: 'Genuine JDM Mazda 13B-REW twin-turbo rotary engine from FD RX-7. Compression tested. Sequential turbo intact.',
    },
    {
        sku: 'ENG-B18C-001',
        ...
        quantity: 5,
        featured: false,
        short_description: 'Honda B18C 1.8L DOHC VTEC engine from JDM Integra Type R. High-revving NA performance classic.',
        description: 'The Honda B18C is one of the most celebrated naturally aspirated performance engines ever built. ' +
        ...
        meta_description: 'JDM Honda B18C 1.8L VTEC engine from Integra Type R. High-rev naturally aspirated powerhouse. Inspected and tested.',
    },
    {
        sku: 'ENG-1JZGTE-001',
        ...
        quantity: 3,
        featured: false,
        short_description: 'Toyota 1JZ-GTE 2.5L turbocharged inline-6 from JDM Mark II / Chaser. Compact powerplant with massive tuning potential.',
        description: 'The Toyota 1JZ-GTE is the smaller sibling of the iconic 2JZ-GTE, offering similar inline-6 smoothness ' +
        ...
        meta_description: 'JDM Toyota 1JZ-GTE 2.5L turbo inline-6 from Mark II and Chaser. Lighter 1JZ alternative. Big tuning potential.',
    },
    {
        ...
        quantity: 1,
        featured: true,
        short_description: 'Nissan RB26DETT 2.6L twin-turbo inline-6 from JDM BNR32/BCNR33/BNR34 Skyline GT-R. The Godzilla engine.',
        description: 'The Nissan RB26DETT is the heart of the legendary Skyline GT-R — known as "Godzilla." ' +
        ...
        meta_description: 'Genuine Nissan RB26DETT 2.6L twin-turbo engine from JDM Skyline GT-R. Rare, inspected, and ready to ship.',
    },
    {
        sku: 'ENG-J35A-001',
        ...
        quantity: 6,
        featured: false,
        short_description: 'Honda J35A 3.5L V6 engine from JDM Odyssey and Pilot. Reliable, smooth daily driver and swap candidate.',
        description: 'The Honda J35A is a 3.5-liter naturally aspirated V6 engine known for its exceptional reliability and smooth power delivery. ' +
        ...
        meta_description: 'JDM Honda J35A 3.5L V6 engine from Odyssey and Pilot. Reliable, low-mileage daily driver replacement.',
    },
    {
        sku: 'ENG-4G63T-001',
        ...
        quantity: 3,
        featured: false,
        short_description: 'Mitsubishi 4G63T 2.0L turbocharged inline-4 from JDM Lancer Evolution. Rally-proven AWD powerhouse.',
        description: 'The Mitsubishi 4G63T is the rally-bred heart of the legendary Lancer Evolution series. ' +
        ...
        meta_description: 'JDM Mitsubishi 4G63T 2.0L turbo engine from Lancer Evolution. Rally-proven cast-iron block. High tuning potential.',
    },
];
// ─── Transmission Data ────────────────────────────────────────────────────────
const transmissions = [
    {
        ...
        quantity: 4,
        featured: false,
        short_description: 'Honda MCTA 5-speed automatic transmission from 2003-2007 Accord. Smooth shifting, direct OEM replacement.',
        description: 'The Honda MCTA is the 5-speed automatic transmission used in the 2003–2007 Honda Accord. ' +
        ...
        meta_description: 'JDM Honda MCTA 5-speed automatic transmission from 2003-2007 Accord. Low mileage, tested, ready to install.',
    },
    {
        sku: 'TRANS-R154-001',
        ...
        quantity: 2,
        featured: true,
        short_description: 'Toyota R154 5-speed manual transmission from JDM Supra. Handles high-torque 2JZ applications with ease.',
        description: 'The Toyota R154 is the iconic 5-speed manual transmission paired with the 2JZ-GTE in the JDM Supra. ' +
        ...
        meta_description: 'JDM Toyota R154 5-speed manual transmission from Supra. Handles 2JZ torque. Inspected synchros and seals.',
    },
    {
        ...
        quantity: 4,
        featured: false,
        short_description: 'Nissan FS5W71C 5-speed manual from JDM Silvia S13/S14/S15. The go-to gearbox for SR20DET builds.',
        description: 'The Nissan FS5W71C is the standard 5-speed manual transmission used in SR20DET-powered Silvia models. ' +
        ...
        meta_description: 'JDM Nissan FS5W71C 5-speed manual gearbox from Silvia. SR20DET companion. Inspected and tested.',
    },
    {
        ...
        quantity: 2,
        featured: false,
        short_description: 'Subaru TY856WB 6-speed manual from JDM WRX STI. Factory close-ratio gearbox for the EJ257 platform.',
        description: 'The Subaru TY856WB is the close-ratio 6-speed manual transmission fitted in the JDM WRX STI from 2004–2014. ' +
        ...
        meta_description: 'JDM Subaru TY856WB 6-speed close-ratio manual from WRX STI. EJ257 platform gearbox. Low mileage.',
    },
    {
        sku: 'TRANS-M15MD-001',
        ...
        quantity: 3,
        featured: false,
        short_description: 'Mazda M15M-D 5-speed manual from JDM FD RX-7. Matched to 13B-REW for optimal rotary performance.',
        description: 'The Mazda M15M-D is the factory 5-speed manual transmission paired with the 13B-REW rotary engine in the FD RX-7. ' +
        ...
        meta_description: 'JDM Mazda M15M-D 5-speed manual from FD RX-7. Perfect 13B-REW companion. Inspected and tested.',
    },
];
...
const blogData = [
    {
        ...
        excerpt: 'From the legendary 2JZ-GTE to the high-revving B18C, we break down the five JDM engines ' +
...
All of these engines are available now at JDM Tokyo Motorsports. Contact us or browse our catalog for current inventory and pricing.`,
        ...
        meta_description: 'Discover the top 5 JDM engines for performance builds: 2JZ-GTE, B18C, SR20DET, 13B-REW, and 4G63T. Expert breakdown from JDM Tokyo Motorsports.',
    ...
    },
    {
        ...
        excerpt: 'Not all JDM engines are equal. Learn what to look for — compression numbers, oil condition, ' +
...
Have questions about a specific engine? [Contact us](/contact) and our team will provide the documentation you need to buy with confidence.`,
        ...
        meta_description: 'Learn how to evaluate JDM engine quality: compression tests, oil inspection, mileage documentation, and auction sheet grading. Expert guide from JDM Tokyo Motorsports.',
    ...
    },
    {
        ...
        excerpt: 'From palletization to freight delivery at your door, here\'s the complete guide to how JDM engines ship ' +
...
For business deliveries (forklift or loading dock available), shipping rates are lower. Select the correct delivery type during checkout to ensure accurate shipping costs.`,
        ...
        meta_description: 'Complete guide to JDM engine shipping: palletization, freight carriers, delivery appointments, and what to check on arrival. From JDM Tokyo Motorsports.',
...
async function seed() {
    ...
    for (const post of blogData) {
        ...
        await database_1.db.insert(schema_1.blogPosts).values({
            slug: post.slug,
            title: post.title,
    ...
    console.log('\n✅ Seed complete.');
    console.log(`   Products: ${enginesInserted + transInserted}/${totalProducts} inserted`);
    console.log(`   Blog posts: ${blogInserted}/${blogData.length} inserted`);
}
...
const POLICY_WARRANTY = [
    ...
    },
    {
        ...
        content: 'The warranty period begins on the date of delivery as confirmed by the freight carrier. Claims must be submitted within 30 calendar days of delivery to be eligible.',
    },
];
const POLICY_SHIPPING = [
        ...
        items: [
            {
                ...
                value: '$500',
                description: 'Delivery to a commercial address with a loading dock or forklift available for unloading.',
                icon: 'building',
            },
            {
                title: 'Residential Address',
                value: '$700',
                description: 'Delivery to a home or residential address. Includes liftgate service for safe unloading.',
                icon: 'home',
            },
        ],
    },
    ...
    },
    {
        ...
        content: 'All engines and transmissions are palletized and wrapped for safe transit. You will need to inspect your shipment upon delivery and note any visible damage on the bill of lading before signing. Contact us immediately if damage is discovered.',
    },
];
const POLICY_RETURNS = [
    {
        type: 'text',
        title: 'Return Policy',
        content: 'We accept returns within 30 days of delivery for units that qualify under our return eligibility. All refunds are processed through Stripe and typically appear within 5–10 business days.\n\nTo initiate a return, please contact our team by phone or email with your order number and reason for return. We will provide return shipping instructions.',
    },
    {
        type: 'bullets',
        icon: 'check',
...
const POLICY_PRIVACY = [
    {
        ...
        content: 'We collect information you provide directly to us, including your name, email address, phone number, shipping address, and payment information when you place an order. We also automatically collect certain information about your device and how you interact with our website.',
    },
    {
        type: 'text',
        title: 'How We Use Your Information',
        content: 'We use the information we collect to process your orders, send order confirmations and shipping notifications, respond to your inquiries, improve our website and services, and send promotional communications if you have opted in.',
    },
    {
        type: 'text',
        title: 'Information Sharing',
        content: 'We do not sell your personal information. We share your information only with service providers who help us operate our business (such as Stripe for payment processing and freight carriers for delivery), and when required by law.',
    },
    {
        type: 'text',
        title: 'Cookies and Tracking',
        content: 'We use cookies and similar tracking technologies to enhance your browsing experience, remember your preferences, and analyze website traffic. You can disable cookies in your browser settings, but some features of the site may not function properly.',
    },
    {
        type: 'highlight',
        icon: 'shield',
        title: 'Data Security',
        content: 'We implement appropriate technical and organizational measures to protect your personal information against unauthorized access, alteration, disclosure, or destruction. Payment information is processed securely by Stripe and is never stored on our servers.',
    },
    {
        type: 'text',
        title: 'Your Rights',
        content: 'You have the right to access, correct, or delete your personal information. You may also opt out of promotional communications at any time by clicking the unsubscribe link in any email or by contacting us directly.',
    },
    {
        type: 'disclaimer',
        icon: 'mail',
...
const POLICY_TERMS = [
    {
        ...
        content: 'By accessing and placing an order with JDM Tokyo Motorsports, you agree to be bound by these Terms of Service and all applicable laws and regulations. If you do not agree, please do not use our website or place orders.',
    },
    {
        type: 'text',
        title: 'Products and Descriptions',
        content: 'We make every effort to display accurate product information, including specifications, mileage, and condition details. All units are sourced from Japanese domestic market vehicles and inspected before listing. Photos are representative and may not depict the exact unit you receive.',
    },
    {
        type: 'text',
        title: 'Orders and Acceptance',
        content: 'All orders are subject to acceptance and availability. We reserve the right to refuse or cancel any order at our discretion, including in the event of pricing errors, suspected fraud, or stock issues. You will be notified and refunded promptly if your order is cancelled.',
    },
    {
        type: 'text',
        title: 'Payment',
        content: 'Payment is processed securely via Stripe at the time of checkout. We accept all major credit and debit cards. Your order will not be shipped until payment has been successfully captured.',
    },
    {
        type: 'text',
        title: 'Shipping',
        content: 'Shipping rates are $500 for business addresses and $700 for residential addresses, shipped via freight carrier. Transit times typically range from 3–7 business days after order processing. See our Shipping page for full details.',
    },
    {
        type: 'text',
        title: 'Returns and Refunds',
        content: 'Returns are accepted within 30 days of delivery for eligible items, as outlined in our Returns and Warranty policies. Refunds are processed through Stripe and may take 5–10 business days to appear on your statement.',
    },
    {
        type: 'highlight',
        icon: 'alert',
        title: 'Limitation of Liability',
        content: 'JDM Tokyo Motorsports is not liable for any damages arising from improper installation, modification, or use of our products. Our maximum liability for any claim is limited to the purchase price of the product.',
    },
    {
        type: 'text',
        title: 'Governing Law',
...
async function upsertSetting(key, value, description, overwriteIfEmpty = false) {
    const existing = await database_1.db
        .select({ id: schema_1.saleConditions.id, rule_value: schema_1.saleConditions.rule_value })
    ...
    if (existing) {
        ...
        if (!isEmpty)
            return 'skipped';
        await database_1.db
            .update(schema_1.saleConditions)
            .set({ rule_value: value, description: description ?? null, updated_at: new Date().toISOString() })
            .where((0, drizzle_orm_1.eq)(schema_1.saleConditions.id, existing.id));
        return 'updated';
    }
...
async function seedMakesAndModels() {
    ...
    for (let i = 0; i < VEHICLE_DATA.length; i++) {
        ...
        const existingMake = await database_1.db
            .select({ id: schema_1.makes.id })
            .from(schema_1.makes)
            .where((0, drizzle_orm_1.eq)(schema_1.makes.name, v.name))
            .get();
        let makeId;
        if (existingMake) {
        ...
        for (const modelName of v.models) {
            const existingModel = await database_1.db
                .select({ id: schema_1.models.id })
                .from(schema_1.models)
                .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.models.name, modelName), (0, drizzle_orm_1.eq)(schema_1.models.make_id, makeId)))
                .get();
            if (existingModel) {
                continue;
            }
...
async function seedNavbarLinks() {
    ...
    const existing = await database_1.db
        .select({ id: schema_1.navbarSettings.id, setting_value: schema_1.navbarSettings.setting_value })
    ...
    if (existing) {
        ...
        await database_1.db
            .update(schema_1.navbarSettings)
...
async function seedProductImages() {
    ...
    const mediumDir = (0, path_1.join)(process.cwd(), 'uploads', 'products', 'medium');
    let pool = [];
    if ((0, fs_1.existsSync)(mediumDir)) {
        pool = (0, fs_1.readdirSync)(mediumDir)
    ...
    const allProducts = await database_1.db.select({ id: schema_1.products.id, sku: schema_1.products.sku }).from(schema_1.products);
    let inserted = 0;
    let skipped = 0;
    let poolIdx = 0;
    for (const p of allProducts) {
        const existing = await database_1.db
            ...
            .where((0, drizzle_orm_1.eq)(schema_1.productImages.product_id, p.id))
            .get();
        if (existing) {
            skipped++;
            continue;
        ...
        poolIdx++;
        await database_1.db.insert(schema_1.productImages).values({
            product_id: p.id,
        ...
        await database_1.db
            ...
            .where((0, drizzle_orm_1.eq)(schema_1.products.id, p.id));
...
async function enrichProductDetails() {
    ...
    const allProducts = await database_1.db.select().from(schema_1.products);
    let updated = 0;
    for (const p of allProducts) {
        const patch = {};
        if (!p.mileage_km) {
        ...
        if (!p.specs_json) {
            const specs = [];
            if (p.engine_code)
                specs.push({ label: 'Engine Code', value: p.engine_code });
            if (p.displacement)
                specs.push({ label: 'Displacement', value: p.displacement });
            if (p.cylinders)
                specs.push({ label: 'Cylinders', value: String(p.cylinders) });
            if (p.fuel_type)
                specs.push({ label: 'Fuel Type', value: p.fuel_type });
            if (p.transmission_type)
                specs.push({ label: 'Transmission Type', value: p.transmission_type });
        ...
        if (Object.keys(patch).length > 0) {
            await database_1.db.update(schema_1.products).set(patch).where((0, drizzle_orm_1.eq)(schema_1.products.id, p.id));
            updated++;
        }
    }
...
async function seedDemoOrders() {
    ...
    const existingOrders = await database_1.db.select({ id: schema_1.orders.id }).from(schema_1.orders).limit(1);
    if (existingOrders.length > 0) {
        console.log('  ⏭  Skipped (orders already exist).');
        return;
    }
    // Get all products to use in order items
    const allProducts = await database_1.db
        .select({ id: schema_1.products.id, title: schema_1.products.title, sku: schema_1.products.sku, price_cents: schema_1.products.price_cents })
        .from(schema_1.products);
    ...
    for (let i = 0; i < ORDER_COUNT; i++) {
        ...
        await database_1.db.insert(schema_1.orders).values({
            id: orderId,
            ...
            payment_status: statusInfo.payment,
        ...
        for (const prod of orderProducts) {
            await database_1.db.insert(schema_1.orderItems).values({
                order_id: orderId,
                product_id: prod.id,
                quantity: 1,
