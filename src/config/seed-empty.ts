...
// ─── Content data (mirrored from data.ts) ────────────────────────────────────
...
function main(): void {
  try {
    ...
    console.log('\nUpserting content into sale_conditions...');
    ...
    console.log('\nSeed-empty complete.');
    console.log('Kept: makes, models, sale_conditions, shipping_zones, email_templates, admin_users, order_notification_recipients, navbar_settings');
    console.log('Cleared: products, product_images, product_compatibility, orders, order_items, reviews, blog_posts, subscriptions, inventory_log, email_log');
  } catch (e) {
    console.error('Error during seed-empty:', e);
    process.exit(1);
  }
}
